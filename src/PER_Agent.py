...
class Agent:
    def __init__(self, config):
        ...
        self.use_priority = config['use_priority'] if 'use_priority' in config.keys(
        ) else True
        self.buffer_size = config['buffer_size'] if 'buffer_size' in config.keys(
        ) else 10
        self.alpha = config['PER_Buffer_Alpha'] if 'PER_Buffer_Alpha' in config.keys(
        ) else 0.1
        self.beta = config['PER_Buffer_Beta'] if 'PER_Buffer_Beta' in config.keys(
        ) else 0.1
        self.eps = config['PER_Buffer_eps'] if 'PER_Buffer_eps' in config.keys(
        ) else 1e-3
        ...
        self.optimizer2 = config['optimizer2'] if 'optimizer' in config.keys(
        ) else torch.optim.Adam(self.model.parameters(), lr=lr)
        ...
        self.tau = config['update_target_tau'] if 'update_target_tau' in config.keys(
        ) else 0.005

        # Mise en place monitoring par MC
        self.monitoring_nb_trials = config['monitoring_nb_trials'] if 'monitoring_nb_trials' in config.keys(
        ) else 0
        self.monitor_every = config['monitor_every'] if 'monitor_every' in config.keys(
        ) else 10
        self.save_every = config['save_every'] if 'save_every' in config.keys(
        ) else 100
        self.double = config['double'] if 'double' in config.keys() else True
        self.save_always = config['save_always'] if 'save_always' in config.keys(
        ) else 20

    def soft_update(self):
        if self.update_target_strategy == 'ema':
            for tp, sp in zip(self.target_model.parameters(),
            ...
            if step > batch_size:
                ...
                if done:
                    ...
                    if episode % test_every == 0:
                        mean, std = self.evaluate_policy(
                            env, episodes=10, seed=seed)
                        validation_score = evaluate_HIV(
                            agent=self, nb_episode=1)
                        episode_return.append(validation_score)

                        print(
                            f"Episode: {episode}, Validation score: {validation_score:4.6g}, Reward mean: {mean:.2f}, Reward std: {std:.2f}, Loss: {total_loss / loss_count:.4f}, Eps: {eps:.3f}")
                        ...
                        if validation_score > best_return:
                            ...
                            torch.save(
                                self.model.state_dict(), "best_raw_{}_model_{}_val_len_{}_{}.pt".format(
                                    self.name, self.time, int(
                                        str(validation_score)[
                                            :2]), np.floor(
                                        np.log10(
                                            np.abs(validation_score))).astype(int)))

                            print(
                                f"Saving model! Current validation score : {validation_score:.6g}\n")
                            print(
                                "Saving model! Best return is updated to ",
                                best_reward)
                            self.previous_raw = "best_raw_{}_model_{}_val_len_{}_{}.pt".format(
                                self.name, self.time, int(
                                    str(validation_score)[
                                        :2]), np.floor(
                                    np.log10(
                    ...
                    if episode % self.save_always == 0:
                        ...
                        if os.path.isdir('/kaggle/working'):
                            print("Kaggle saving")
                            torch.save(self.saved_model.state_dict(
                            ), "/kaggle/working/saved_model_kaggle_{}_{}.pt".format(self.name, self.time))
                            # torch.save(self.saved_model.state_dict(), "saved_model_kaggle_{}.pt".format(self.time))
                        else:
                            print("basic saving.")
                            torch.save(
                                self.saved_model.state_dict(),
                                "saved_model_{}.pt".format(
                                    self.time))
