...
class NoisyLinear(nn.Module):
    # Noisy Linear Layer for factorised Gaussian Noise, for better
    ...
    def __init__(self, in_features, out_features, sigma_init=0.5):
        ...
        # extra parameter for the bias and register buffer for the bias
        # parameter
    ...
    def scale_noise(self, size):
        # generate 0-mean noise with fixed statistic (Gaussian(O,1))
        x = torch.randn(size)
...
class Agent:
    def __init__(self, config):
        self.model = MLP(
            ...
            hidden_dim=config['nb_neurons'],
            output_dim=config['nb_actions'],
            depth=config['hidden_layers'],
            activation=nn.ReLU(),
            normalization='None').to(device)
    ...
    def gradient_step_double(self):
        if len(self.memory) > self.batch_size:
            ...
            Q_target_XA = self.target_model(X).gather(
                1, A.to(torch.long).unsqueeze(1))
    ...
    def V_initial_state(self, env, nb_trials):
        with torch.no_grad():
            for _ in range(nb_trials):
                ...
                val.append(
                    self.model(
                        torch.Tensor(x).unsqueeze(0).to(device)).max().item())
        return np.mean(val)
        ...
        for _ in tqdm(generator()):
            ...
            if done or trunc:
                ...
                if validation_score > best_return:
                    ...
                    print(
                        "Saving raw model! Best return is updated to ",
                        best_return)
